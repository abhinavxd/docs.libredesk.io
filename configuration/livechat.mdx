...
| `phone_number_country_code` | No | ISO 3166-1 alpha-2 country code for the phone number (e.g. `IN`, `US`). |
...
Contacts don't have to stay in the live chat. If they leave before the conversation is finished, Libredesk can automatically email them any unread messages.


| Setting                  | Description                                                                        |
| ------------------------ | ---------------------------------------------------------------------------------- |
| `offline_threshold`      | How long agents must be offline before Libredesk sends a fallback email (e.g. `10m`). |
| `max_messages_per_email` | Maximum number of messages per email. Additional messages are sent in a new email. |
| `min_email_interval`     | Minimum time between fallback emails for the same conversation.                    |

Replies to the email thread are added back to the same live chat conversation.
...
Leave this empty to allow all origins. Always set it in production.
